...
// Only track mouse movement and apply wind effects on non-touch devices
if (!isTouchDevice) {
  let rafId;
...
const gravity = 0.002; // minimal gravity applied to every star
...
class Star {
  ...
  update() {
    // Only apply wind influence on non-touch devices
    if (!isTouchDevice) {
    ...
    // Wrap-around logic for continuous background
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  links.forEach((link) => {
    link.addEventListener("click", (e) => {
      ...
      const oldAnimatedDivs =
        currentSection.querySelectorAll(".content-animation");
      ...
      setTimeout(() => {
        ...
        const animatedDivs =
          targetSection.querySelectorAll(".content-animation");
...
function toggleModal(modalID) {
  ...
  const modalContent = modal.querySelector(`#${modalID} > div > div`);
